...
static struct gui_clipboard {
	char buffer[MAXSNARF+1];
	size_t length;
} gui_clipboard;

/* write to the plan 9 snarf buffer */

void
putsnarf(char *s, int len)
{
	int fd;
...
/* read from the plan 9 snarf buffer into gui_clipboard */
...
getsnarf()
{
	...
	if(fd < 0)
		return 0;
	n = read(fd, gui_clipboard.buffer, sizeof(gui_clipboard.buffer));
	close(fd);
	if(n < 0)
		n = 0;			/* some error reading */
	gui_clipboard.length = n;
	gui_clipboard.buffer[n] = '\0';	/* terminate string */
	return n;	
}
 ...
 */
static void
gui_get_clipboard(char **buffer, size_t *length)
{
	*buffer = NULL;
	*length = 0;
	
	getsnarf();	/* reads into gui_clipboard */
 ...
 */
static void
gui_set_clipboard(const char *buffer, size_t length,
		nsclipboard_styles styles[], int n_styles)
{
	putsnarf(buffer, length);
}
